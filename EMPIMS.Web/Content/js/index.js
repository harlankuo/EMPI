...
$(function () {
    ...
    $("#content-main iframe").attr("src", "/Home/Default");
});
...
function createIframe() {
    ...
    i.height = "100%";
    i.width = "100%";
    i.class = "NFine_iframe";
    i.border = 0;
    document.getElementById("content-main").appendChild(i);
...
function LoadMenue() {
    ...
    if (menue.length > 0) {
        ...
        for (var i = 0; i < menue.length; i++) {
            if (menue[i].FuncType == "2") {

                funcCode = menue[i].FuncCode;
                if (funcCode.lastIndexOf(".") >= 0) {
                    url = "/IndexManage/" + funcCode.split(".")[1] + "/Index";
                }
                strHtml += "<li><a class=\"menuItem\" data-id=\"" + menue[i].FuncID + "\" href=\"" + url + "\">";
                strHtml += "<i class=\"" + menue[i].FuncIcon + "\"></i>";
                strHtml += "<span>" + menue[i].FuncName + "</span>";
                strHtml += "</a></li>";
            }
        }
    }

    $("#empims_menue").append(strHtml);
}
