(function ($) {
    $.nfinetab = {
        ...
        addTab: function () {
            ...
            if (dataId != "") {
                $.cookie('nfine_currentmoduleid', dataId, { path: "/" });
            }
        ...
        addTabSingle: function (_id, _url, _title) {

            parent.$("#header-nav>ul>li.open").removeClass("open");
            var dataId = _id;
            if (dataId != "") {
                $.cookie('nfine_currentmoduleid', dataId, { path: "/" });
            }
            ...
            parent.$('.menuTab').each(function () {
                if ($(this).data('id') == dataUrl) {
                    if (!$(this).hasClass('active')) {
                        $(this).addClass('active').siblings('.menuTab').removeClass('active');
                        $.nfinetab.scrollToTab(this);
                        parent.$('.mainContent .NFine_iframe').each(function () {
                            if ($(this).data('id') == dataUrl) {
                                $(this).show().siblings('.NFine_iframe').hide();
                                return false;
                            }
            ...
            if (flag) {
                ...
                var str1 = '<iframe class="NFine_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
                parent.$('.mainContent').find('iframe.NFine_iframe').hide();
                ...
                $.nfinetab.scrollToTab(parent.$('.menuTab.active'));
            }
            return false;
        },
        init: function () {
