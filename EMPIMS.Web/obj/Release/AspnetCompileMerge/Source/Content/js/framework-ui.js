...
$.modalAlertCallback = function (content, type, callback) {
    var icon = "";
    if (type == 'success') {
        icon = "fa-check-circle";
    }
    ...
    var layer = top.layer.alert(content, {
        icon: icon,
        title: "系统提示",
        closeBtn: 0,
        btn: ['确认'],
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state == "success") {
                    ...
                    $.modalAlertCallback(data.message, data.state, function () {
                        options.success(data);
                        if (options.close == true) {
                            $.modalClose();
                        }
...
$.getAjax = function (url, postData, callBack) {
    $.ajax({
        type: 'post',
        dataType: "text",
        url: url,
...
$.AjaxJson = function (url, postData, callBack) {
    $.ajax({
        url: url,
        type: "post",
        data: postData,
...
$.oneRowOnly = function (value) {
    if (value == null || value == undefined || value == "") {
        $.modalMsg("您没有选择任何项！", "warning");
        return false;
    } else {
        if (value.indexOf(",") > 0) {
            $.modalMsg("只能选择一行数据！", "warning");
            return false;
        } else {
            return true;
        }
...
$.fn.jqGridSelectNameValue = function (name) {
    var $grid = $(this);
    var selectedRowIds = $grid.jqGrid("getGridParam", "selarrrow");
    var names = "";
    if (selectedRowIds != "") {
...
$.fn.bindSelect = function (options) {
    ...
    if (options.url != "" && options.data == null) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (options.defaultText != "") {
                    $element.append($("<option></option>").val("").html(options.defaultText));
                }
        ...
        minimumResultsForSearch: options.search == -1;
    }
}
...
$.fn.dataGrid = function (options) {
    ...
    options["onSelectRow"] = function (rowid) {
        ...
        if (!!selrow) {
            length = $(this).jqGrid("getGridParam", "selrow").length;
        }
